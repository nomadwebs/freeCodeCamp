...
- El primer argumento se conoce como acumulador, que recibe el valor retornado de la función callback de la iteración anterior, 
...
const watchList = [
  ...
  function getRating(watchList) {
    ...
    let listaRatings = watchList
    .filter(movie => movie.Director === "Christopher Nolan"); //Primero aplicamos el filter para asegurarnos que tenemos todos los datos.
    console.log(listaRatings);
   
    // Usamos map para obtener solo las calificaciones (ratings) en formato numérico
    let ratings = listaRatings.map(movie => parseFloat(movie.imdbRating));
    console.log(ratings);

    //Ahora ya podemos calcular la media: 
    let sumaRatings = ratings.reduce((sum, rating) => sum + rating, 0);
    console.log(sumaRatings);
    
    averageRating = sumaRatings / ratings.length;
